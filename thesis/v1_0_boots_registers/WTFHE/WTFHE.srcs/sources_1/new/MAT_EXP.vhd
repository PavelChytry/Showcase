...
architecture MAT_EXP_BODY of MAT_EXP is
...
begin
    ...
    gen_j:
    for i_j in 0 to K generate
        gen_i:
        for i_i in 0 to (K + 1) * L - 1 generate
            mem : NEGACYCLICMEM port map (CLK => CLK, RST => RST, LOAD => START, PULSE => PULSE, DIR => DIR, A => MAT_IN(i_j)(i_i), Y => MAT_OUT(i_j)(i_i));
        end generate;
    end generate;
	
	PAD(NU + 1 downto 0) <= EXP;
