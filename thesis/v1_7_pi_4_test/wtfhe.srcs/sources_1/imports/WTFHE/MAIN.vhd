...
begin
	...
	boots : BOOTSTRAPPING_RAM port map (CLK => CLK_NEW, RST => RST, START => BOOT_START_DELAY--BOOT_START
	, RAM_WE => RAM_WE, TLWE_IN => TLWE_DELAY--TLWE_IN
	, RAM_DATA => RAM_DATA, RAM_WE_ADDR => RAM_WE_ADDR, TLWE_OUT => TLWE_OUT, DONE => BOOT_DONE);
...
--    clkgen : clk_wiz_0 port map (clk_out1 => CLK_NEW, reset => HW_RST, locked => open, clk_in1 => CLK);
...
--	--this might be weird
--	--SPLIT_IN_U <= (NO_OF_INPUT_BITS - TAU - 1 downto 0 => '0') & TLWE_OUT(to_integer(CNT_R));
...
--    --test purposes
--	SPLT: for I_a in NU - 1 downto 0 generate
--	   TLWE_OUT(I_a) <= TLWE_IN(I_a);
--	end generate;
--	BOOT_DONE <= '1';
--	DATA <= to_unsigned(MATRIX_HIGH, ADDR_WIDTH);
	
--	RAM_DATA <= MERGE_OUT_U;
--	GEN_R: for I_a in 0 to NU - 1 generate
--	   reg_p : REG generic map (REG_SIZE => TAU)
--	       port map (CLK => CLK_NEW, RST => RST, 
--		      WE => WE_R(I_a),
--              A => MERGE_OUT_U(TAU - 1 downto 0),
...
--	--transition function
--	process (STATE, RECEIVED, CNT_ZERO, BOOT_DONE, SENT, MERGE_OUT_U)
--	begin
--	   NEXT_STATE <= STATE; 
--	   SW_RST <= '0';
...
--	                   when to_unsigned(sw_RESET, NO_OF_INPUT_BITS) =>
--	                       NEXT_STATE <= RESET;
--	                   when others => NEXT_STATE <= RESET;--null;
--	               end case;
--	           end if;
--	       when LOAD_TRGSW =>
--	           if RECEIVED = '1' then
--	               if MERGE_OUT_U = sw_RESET then
--	                   NEXT_STATE <= RESET;
--	               elsif CNT_ZERO = '1' then
--	                   NEXT_STATE <= INIT;
--	               end if;
...
--	--output function
----	process (STATE, RECEIVED, CNT_ZERO, BOOT_DONE, SENT, MERGE_OUT_U, CNT_R)
----	begin
----	   BOOT_START <= '0';
----	   RST_CNT <= '0';
----	   PULSE_CNT <= '0';
...
----	                   when to_unsigned(send_INPUT, NO_OF_INPUT_BITS) =>
----	                       RAM_SET_ADDR <= to_unsigned(MATRIX_HIGH + 1, ADDR_WIDTH);
----	                       DATA <= to_unsigned(POLY_START - MATRIX_HIGH, ADDR_WIDTH);
----	                       RST_CNT <= '1';
----                           ADDR_REG_RST <= '1';
----	                   when to_unsigned(sw_RESET, NO_OF_INPUT_BITS) =>
----	                       NULL;
----	                   when others => NULL;
----	               end case;
------	               if MERGE_OUT_U = to_unsigned(send_TRGSW, NO_OF_INPUT_BITS) then
------                        DATA <= to_unsigned(MATRIX_HIGH, ADDR_WIDTH);
------                        RST_CNT <= '1';
------                        ADDR_REG_RST <= '1';
------	               elsif MERGE_OUT_U = to_unsigned(send_INPUT, NO_OF_INPUT_BITS) then
------	                    RAM_SET_ADDR <= to_unsigned(MATRIX_HIGH + 1, ADDR_WIDTH);
------	                    DATA <= to_unsigned(POLY_START - MATRIX_HIGH, ADDR_WIDTH);
------	                    RST_CNT <= '1';
------                        ADDR_REG_RST <= '1';
------	               elsif MERGE_OUT_U = to_unsigned(sw_RESET, NO_OF_INPUT_BITS) then
------	                   NULL;
------	               end if;
----	           end if;
----	       when LOAD_TRGSW =>
...
----	                   NULL;
----	               elsif CNT_ZERO = '1' then
----	                   DATA <= to_unsigned(NU - 1, ADDR_WIDTH);
----	                   RST_CNT <= '1';
----	                   RAM_WE <= '1';
----	               end if;
----	           else
...
----	       when STARTUP => NULL;
----	       when INIT =>
----	           DATA <= to_unsigned(POLY_START, ADDR_WIDTH);
----	           RST_CNT <= '1';
----	           ADDR_REG_RST <= '1';
----	       when LOAD_RAM =>
----	           if RECEIVED = '1' then
