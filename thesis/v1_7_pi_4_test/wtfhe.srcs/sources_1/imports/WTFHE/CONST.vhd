...
package CONST is
	-- WTFHE parameters
	constant GAMMA							: integer := 1; --3
	constant DIM							: integer := 4; --4 -- n
	constant K								: integer := 1; --1
	constant L								: integer := 17; --4
	constant N								: integer := 64; --16
	constant CNT_WIDTH					    : integer := 6; -- 2^CNT_WIDTH == N
	constant IOTA							: integer := GAMMA * L; --12 --15 novì
	constant TAU							: integer := 24; --18 -- 20 novì
	constant NU                             : integer := 6; --4, 2^CNT_WIDTH == N
	
	-- pi = 3 : gamma = 3, l = 5, N = 32, TAU = 22, NU = 5
	-- poly_addr = 16#F3F#, mat_high = 16#EFF#, mat_low = 16#77F#
	-- BRAM parameters
	constant DATA_WIDTH                    : integer := 32; -- WIDTH >= TAU
	constant ADDR_WIDTH                    : integer := 15; -- 2^ADDR_WIDTH >= (DIM * 3 / 2) * (K + 1) * ((K + 1) * L) * N
	constant POLY_START                    : integer := 16#667F#;--16#F3F#; -- highest RAM address
	constant MATRIX_HIGH                   : integer := 16#65FF#;--16#EFF#; -- last address of last matrix
	constant MATRIX_LOW                    : integer := 16#32FF#;--16#77F#; -- last adress of 3rd matrix
