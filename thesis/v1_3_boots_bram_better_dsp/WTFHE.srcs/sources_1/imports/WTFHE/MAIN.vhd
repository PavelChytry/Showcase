...
--	end component;
	...
	signal RAM_WE_ADDR : UNSIGNED(10 DOWNTO 0);
...
begin
    ...
    cntr : EXTENDED_COUNTER port map (CLK => CLK, RST => RST_CNT, CE => PULSE_CNT, SET => DATA, CNT_R => CNT_R, ZERO => CNT_ZERO);
	...
	boots : BOOTSTRAPPING_RAM port map (CLK => CLK,
	 RST => RST,
	  START => BOOT_START, RAM_WE => RAM_WE, TLWE_IN => TLWE_IN,
	RAM_DATA => RAM_DATA, RAM_WE_ADDR => RAM_WE_ADDR, TLWE_OUT => TLWE_OUT, DONE => BOOT_DONE);
    
	a_reg : ADDR_REG port map (CLK => CLK, RST => ADDR_REG_RST, CE => ADDR_CE, DIR => '1', PASS => '0', SET => (others => '0'), PASSTHROUGH => open, Y => RAM_WE_ADDR);
