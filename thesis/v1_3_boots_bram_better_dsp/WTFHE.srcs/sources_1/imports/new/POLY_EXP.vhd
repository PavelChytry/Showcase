    ...
    begin
        if rising_edge(CLK) then
            if RST = '1' then
                ...
                if FLAG = '1' then
                    P_OUT(to_integer(CNT_R(CNT_WIDTH downto CNT_WIDTH)))(to_integer(IDX(3 downto 0))) <= not(DATA) + 1;
                else
                    P_OUT(to_integer(CNT_R(CNT_WIDTH downto CNT_WIDTH)))(to_integer(IDX(3 downto 0))) <= DATA;
                end if;
            end if;
        end if;
    end process;
