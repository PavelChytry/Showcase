...
architecture SPLITTER_BODY of SPLITTER is
	...
	signal REG : std_logic_vector(NO_OF_INPUT_BITS - 1 downto 0);
	
begin

	process (CLK)
		variable CNT : integer range 0 to NO_OF_INPUT_BITS / NO_OF_TRANSFERRED_BITS + 1;
	begin
		if rising_edge(CLK) then
			if RST = '1' then
				...
				if TXD_READY = '1' then
					EDGE <= not EDGE;
					if EDGE = '1' then
						if CNT = (NO_OF_INPUT_BITS / NO_OF_TRANSFERRED_BITS) then
							CNT := 0;
							ACTIVE <= '0';
							DONE <= '1';
						else
							TXD_DATA <= REG(NO_OF_INPUT_BITS - CNT * 8 - 1 downto NO_OF_INPUT_BITS - CNT * 8 - 8);
							CNT := CNT + 1;
							TXD_STROBE <= '1';
						end if;
					end if;
