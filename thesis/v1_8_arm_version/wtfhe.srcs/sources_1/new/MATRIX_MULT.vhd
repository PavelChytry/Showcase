...
architecture MATRIX_MULT_BODY of MATRIX_MULT is
	...
	signal CNT_R, CNT_L : UNSIGNED(ADDR_WIDTH - 1 downto 0);
...
begin
	...
	l_cnt : EXTENDED_COUNTER port map (CLK => CLK, RST => CNT_RST, CE => PULSE_CNT_L, CNT_R => CNT_L, ZERO => ZERO_L, SET => to_unsigned((K + 1) * L - 1, ADDR_WIDTH));
	...
	l_mult : POLY_MULT port map (CLK => CLK, RST => RST, START => MULT_START_L, I => I_IN_L, T => T_IN_L, Y => T_OUT_L, DONE => MULT_DONE_L);
	...
	r_cnt : EXTENDED_COUNTER port map (CLK => CLK, RST => CNT_RST, CE => PULSE_CNT_R, CNT_R => CNT_R, ZERO => ZERO_R, SET => to_unsigned((K + 1) * L - 1, ADDR_WIDTH));
	...
	r_mult : POLY_MULT port map (CLK => CLK, RST => RST, START => MULT_START_R, I => I_IN_R, T => T_IN_R, Y => T_OUT_R, DONE => MULT_DONE_R);
	
	-- barrier synchronization of both halves
